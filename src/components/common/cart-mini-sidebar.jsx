                ...
                <div key={item._id} className="cartmini__widget-item">
                  ...
                  <div className="cartmini__content">
                    ...
                    <div className="cartmini__price-wrapper">
                      {item.discount > 0 ? <span className="cartmini__price">${(Number(item.price) - (Number(item.price) * Number(item.discount)) / 100)}</span> : <span className="cartmini__price">${item.price}</span>}
                      <span className="cartmini__quantity">{" "}x{item.orderQuantity}</span>
                    </div>
                  </div>
                  <Link href={"#"} onClick={() => handleRemovePrd({ title: item.title, id: item._id })} className="cartmini__del cursor-pointer"><i className="fa-regular fa-xmark"></i></Link>
                </div>
              ))}
            </div>}
            {/* if no item in cart */}
